...
{ 
  ...
  printf("sau khi xu ly\n");
  struct vm_rg_struct *tmp = mm->mmap->vm_freerg_list;
  while(tmp != NULL){
    printf("free_start: %d\n", tmp->rg_start);
    printf("free_end: %d\n", tmp->rg_end);
    tmp = tmp->rg_next;
...
//check xem có bị cấp vượt quá maxsz của ram
int check_ramsz_valid(struct pcb_t *caller, int size){
  struct vm_rg_struct *rgit = caller->mm->mmap->vm_freerg_list;

  struct vm_rg_struct *save_rgit = rgit;
  if(rgit != NULL){
    while(rgit != NULL){
      if (rgit->rg_start + size <= caller->mram->maxsz && rgit->rg_start + size <= rgit->rg_end){
        // trường hợp size chuẩn bị thêm vừa trong một free region nào đó.
        return 0;
      }
      else if (rgit->rg_start + size > caller->mram->maxsz){
        // trường hợp size chuẩn bị thêm vượt quá maxsz của ram
        return -1;
      }
      rgit = rgit->rg_next;
    }
    ...
    if (caller->mm->mmap->sbrk + size > caller->mram->maxsz) return -1;
  }
  return 0;
}
...
{ 
  if (check_ramsz_valid(caller, size) == -1){
