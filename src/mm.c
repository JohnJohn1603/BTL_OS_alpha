...
{                                                   // no guarantee all given pages are mapped
  ...
  int end_addr = addr + pgnum * PAGING_PAGESZ;
  ...
  */
  ...
  for (pgit = 0; pgit < pgnum; pgit++){
    ...
    // int curr_addr = addr + pgit * PAGING_PAGESZ;
    int curr_pgn = pgn + pgit;

    // uint32_t * pte = &caller->mm->pgd[curr_pgn];
  ...
  */
   ...
   */
    ...
    { // TODO: ERROR CODE of obtaining somes but not enough frames
      ...
      //   if(__swap_cp_page(caller->mram, vicpgn, caller->active_mswp, fpn) == -1) return -1;
      //   newfp_str = malloc(sizeof(struct framephy_struct));
      //   if (newfp_str == NULL) return -1;
      //   newfp_str->fpn = vicpgn;
      ...
      // else{
        ...
        while(curr != NULL){
          next = curr->fp_next;
          MEMPHY_put_freefp(caller->mram, curr->fpn);
          free(curr);
          curr = next;
        }
        return -3000;
